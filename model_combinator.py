...
def model_combine():
    ...
    for i in range(3,66):
        path = "wikipedia/" + str(i) + "/learned_data_" + str(i) + ".json"
        with open(path, "r") as f:
            model_json = f.read()
            model_get = markovify.Text.from_json(model_json)
        with open(last_path) as f:
            model_comb = f.read()
            model_comb = markovify.Text.from_json(model_comb)
        model_comb = markovify.combine(models=[model_comb, model_get])
        sentence = model_comb.make_short_sentence(200, 50, tries=10)
        print(sentence)
        with open(last_path, "w") as f:
            f.write(model_comb.to_json())
    ...
    #テスト用
    sentence = model_ocmb.make_sentence()
